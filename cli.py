...
from config import ORIG_DIR, LOG_DIR, TMP_DIR, DEFAULT_SD_BITRATE
...
def convert_file(path: Path, output_dir: Optional[Path] = None, keep_original: bool = False, verbose: bool = False) -> bool:
    ...
    if verbose:
        print(f"  Target bitrate: {target_kbps} kbps (scale: {scale})")

    # Repair pipeline
    if codec == "mpeg1":
        if verbose:
            print("  Repairing MPEG-1...")
    ...
    if verbose:
        print(f"  Encoding to: {out}")
    
    # Encode
    try:
        encode(repaired, out, target_kbps)
    except Exception as e:
        print(f"ERROR: Encoding failed for {path}: {e}")
        return False
    ...
    if not keep_original:
        ...
        if verbose:
            print(f"  Moved original to: {ORIG_DIR / path.name}")
