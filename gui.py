...
from tkinter import Tk, Frame, Label, Button, Entry, Text, Scrollbar, Checkbutton
...
class MediaConverterGUI:
    ...
    def _build_ui(self) -> None:
        ...
        subtitle_label = Label(
            title_frame,
            text="Repair and encode legacy video formats",
            font=("Arial", 10)
        )
        subtitle_label.pack()
        ...
        Label(input_frame, text="Input Path:", width=12, anchor='w').grid(row=0, column=0, sticky='w')
        Entry(input_frame, textvariable=self.input_path, width=50).grid(row=0, column=1, padx=5)
        Button(input_frame, text="Browse File", command=self._browse_file).grid(row=0, column=2, padx=2)
        Button(input_frame, text="Browse Folder", command=self._browse_folder).grid(row=0, column=3, padx=2)
        
        # Output section
        output_frame = Frame(self.root, padx=20, pady=5)
        output_frame.pack(fill='x')
        
        Label(output_frame, text="Output Path:", width=12, anchor='w').grid(row=0, column=0, sticky='w')
        Entry(output_frame, textvariable=self.output_path, width=50).grid(row=0, column=1, padx=5)
        Button(output_frame, text="Browse", command=self._browse_output).grid(row=0, column=2, padx=2)
        Button(output_frame, text="Clear", command=lambda: self.output_path.set("")).grid(row=0, column=3, padx=2)
        ...
        Checkbutton(
            options_frame,
            text="Recursive (process subdirectories)",
        ...
        Label(stats_frame, textvariable=self.success_count, fg="green", font=("Arial", 10, "bold")).pack(side='left')
        
        Label(stats_frame, text="Failed:").pack(side='left', padx=20)
        Label(stats_frame, textvariable=self.fail_count, fg="red", font=("Arial", 10, "bold")).pack(side='left')
    ...
    def _browse_file(self) -> None:
        ...
        filename = filedialog.askopenfilename(
            ...
            filetypes=[
                ("Video Files", "*.avi *.mpg *.mpeg *.wmv *.mov"),
                ("All Files", "*.*")
            ]
        )
        if filename:
    ...
    def _run_conversion(self) -> None:
        ...
        try:
            ...
            if self.fail_count.get() == 0:
                messagebox.showinfo("Success", f"All conversions completed successfully!\n\nConverted: {self.success_count.get()} file(s)")
            else:
                messagebox.showwarning("Completed with Errors", f"Conversion finished with some errors.\n\nSuccessful: {self.success_count.get()}\nFailed: {self.fail_count.get()}")
